...
class HeadFirstOrderingTestCase(TestCase):
    """Upstream promotes whichever member is created first to HEAD when the group has
    none yet, then nulls that member's role when the real head arrives. Emitting the
    head first means the stand-in is never appointed."""
    ...
    ROWS = [
        {"id": "1", "fn": "Child", "ln": "Banda", "d": "2010-01-01", "hh": "H1", "role": "SON"},
        {"id": "2", "fn": "Mary", "ln": "Banda", "d": "1980-01-01", "hh": "H1", "role": "HEAD"},
        {"id": "3", "fn": "Kid", "ln": "Banda", "d": "2012-01-01", "hh": "H1", "role": "DAUGHTER"},
    ]

    def test_head_is_emitted_before_the_other_members(self):
        out = self._adapter().transform(self.ROWS)
        self.assertEqual([r["first_name"] for r in out], ["Mary", "Child", "Kid"])

    def test_order_of_non_heads_is_preserved(self):
        out = self._adapter().transform(self.ROWS)
        self.assertEqual([r["first_name"] for r in out][1:], ["Child", "Kid"])

    def test_can_be_switched_off(self):
        out = self._adapter(head_first=False).transform(self.ROWS)
        self.assertEqual([r["first_name"] for r in out], ["Child", "Mary", "Kid"])

    def test_batch_without_a_head_is_unchanged(self):
        rows = [r for r in self.ROWS if r["role"] != "HEAD"]
        out = self._adapter().transform(rows)
        self.assertEqual([r["first_name"] for r in out], ["Child", "Kid"])
